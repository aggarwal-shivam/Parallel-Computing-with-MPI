...
int main(int argc, char* argv[]){
	...
	double *data, *reduce_result, *reduce_result_final;
	double *gather_result,*gather_result_final;
	double *alltoallv_recv_buffer_final,*alltoallv_recv_buffer;
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%//
	...
	for(int i=0;i<5;i++){
		...
		if(inter_color==0)
		{
			MPI_Reduce(reduce_result,reduce_result_final,doubles,MPI_DOUBLE,MPI_SUM,0,inter_comm);
		}
	}
	end=MPI_Wtime();
...
// //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%//
	...
	for(int i=0;i<5;i++){
		...
		if(inter_color==0)
		{
			MPI_Gather(gather_result,ppn*doubles,MPI_DOUBLE,gather_result_final,ppn*doubles,MPI_DOUBLE,0,inter_comm);
		}
	}
	end=MPI_Wtime();
...
// //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%//
	...
	// printf("entering optim all to all\n myrank=%d\n",myrank);
	start=MPI_Wtime();
	for(int i=0;i<5;i++){
		...
		if(inter_color ==0)
		{
			MPI_Allgather(alltoallv_recv_buffer,ppn*doubles,MPI_DOUBLE,alltoallv_recv_buffer_final,ppn*doubles,MPI_DOUBLE,inter_comm);
		}
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%//
	...
	for(int i=0;i<4;i++)
	{
		MPI_Reduce(&optimized_time[i],&max_optimum[i],1,MPI_DOUBLE,MPI_MAX,0,MPI_COMM_WORLD);
		MPI_Reduce(&default_time[i],&max_default[i],1,MPI_DOUBLE,MPI_MAX,0,MPI_COMM_WORLD);
	}
	if(myrank==0){
		for(int i=0;i<4;i++){
